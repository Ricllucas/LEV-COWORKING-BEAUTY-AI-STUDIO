...
export const AgendaView: React.FC<AgendaViewProps> = ({ currentUser, onOpenNewBooking }) => {
  ...
  const [selectedProfFilter, setSelectedProfFilter] = useState<string>(
    currentUser.role === 'profissional' && currentUser.professionalId ? currentUser.professionalId : 'todas'
  );
      ...
      <div className="p-4 bg-[#0a0a0a] rounded-2xl border border-white/10 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            disabled={currentUser.role === 'profissional'}
        ...
        {filteredAppointments.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-base font-serif font-medium text-white">
              Nenhum agendamento para a seleção atual
            </p>
            <p className="text-xs text-white/60 mt-1 max-w-sm mx-auto">
              Utilize o botão de "Novo Agendamento" ou altere os filtros de data e profissional.
            </p>
          </div>
        ) : (
            ...
            {filteredAppointments.map(apt => {
              ...
              return (
                <div
                  ...
                  className="p-4 rounded-xl border border-white/10 bg-[#050505] hover:border-[#c4b491] transition-all cursor-pointer flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-2xs"
                >
                  <div className="flex items-start sm:items-center gap-3">
                    <div
                      className="px-3 py-2 rounded-xl text-xs font-bold text-black text-center shrink-0 shadow-2xs"
                      style={{ backgroundColor: prof?.color || '#c4b491' }}
                    >
                      <span>{apt.startTime}</span>
                      <span className="block text-[10px] font-normal opacity-90">{apt.endTime}</span>
                    </div>

                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-sm text-white">{apt.clientName}</span>
                        <span className="text-[11px] text-[#c4b491]">({apt.clientPhone})</span>
                      </div>
                      <span className="text-xs text-white/60 block mt-0.5">
                        {apt.serviceNames.join(', ')} • <strong className="text-white">{apt.professionalName}</strong>
                      </span>
                    </div>
                  </div>

                  <div className="flex items-center gap-3 justify-between sm:justify-end border-t sm:border-0 pt-2 sm:pt-0 border-white/10">
                    <span className={`text-[10px] px-2.5 py-1 rounded-full font-medium border uppercase tracking-wider ${
                      apt.status === 'confirmado' ? 'bg-emerald-950/60 text-emerald-300 border-emerald-800' :
                      apt.status === 'cliente_presente' ? 'bg-blue-950/60 text-blue-300 border-blue-800' :
                      apt.status === 'concluido' ? 'bg-stone-800 text-stone-300 border-stone-600' :
